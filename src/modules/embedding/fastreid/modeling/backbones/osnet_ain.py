...
class LightConvStream(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bn_norm, depth):
        super(LightConvStream, self).__init__()
        assert depth >= 1, 'depth must be equal to or larger than 1, but got {}'.format(
            depth
        )
        layers = []
        layers += [LightConv3x3(in_channels, out_channels, bn_norm)]
        for i in range(depth - 1):
            layers += [LightConv3x3(out_channels, out_channels, bn_norm)]
...
class OSBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bn_norm, reduction=4, T=4, **kwargs):
        ...
        for t in range(1, T + 1):
            self.conv2 += [LightConvStream(mid_channels,
                                           mid_channels, bn_norm, t)]
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.conv2 = self._make_layer(
            blocks[0], layers[0], channels[0], channels[1], bn_norm, IN=IN
        )
        self.pool2 = nn.Sequential(
            Conv1x1(channels[1], channels[1],
                    bn_norm), nn.AvgPool2d(2, stride=2)
        )
        self.conv3 = self._make_layer(
            blocks[1], layers[1], channels[1], channels[2], bn_norm
        )
        ...
        self.conv4 = self._make_layer(
            blocks[2], layers[2], channels[2], channels[3], bn_norm
        )
        self.conv5 = Conv1x1(channels[3], channels[3], bn_norm)

        self._init_params()

    def _make_layer(self, blocks, layer, in_channels, out_channels, bn_norm, IN=False):
        layers = []
        layers += [blocks[0](in_channels, out_channels, bn_norm, IN=IN)]
        for i in range(1, len(blocks)):
            layers += [blocks[i](out_channels, out_channels, bn_norm, IN=IN)]
...
def build_osnet_ain_backbone(cfg):
    """
    ...
    model = OSNet([[OSBlockINin, OSBlockINin], [OSBlock, OSBlockINin],
                   [OSBlockINin, OSBlock]],
